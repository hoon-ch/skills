...
Use this skill when the work is a landing page, portfolio, or redesign and the risk is a templated, obviously-AI result. Skip it for dashboards, data tables, and multi-step product UI (Section 13).

1. Infer the brief and state a one-line Design Read (Section 0).
2. Set `DESIGN_VARIANCE`, `MOTION_INTENSITY`, and `DENSITY` (Sections 1 and 7).
3. Map the brief to a real design system instead of inventing one (Sections 2 and Appendix A).
4. Build against the architecture, directives, and block contract (Sections 3, 4, 12).
5. Run the pre-flight check before shipping (Section 14).

## Workflow

| Stage | Read | Output |
| --- | --- | --- |
| Read the room | Section 0 | one-line Design Read, at most one clarifying question |
| Configure | Sections 1, 7 | three dial values |
| Choose the system | Section 2, Appendix A, Appendix B | named design system or an explicit custom decision |
| Audit first (redesigns only) | Section 11 | preserve/overhaul call backed by existing brand assets |
| Build | Sections 3, 4, 12 | components that satisfy the block contract |
| Correct bias | Sections 5, 9 | no forbidden AI tells |
| Guard | Sections 6, 8 | performance, accessibility, dark mode |
| Ship | Section 14 | completed pre-flight check |

## Failure Fallback

- **Brief genuinely ambiguous**: ask exactly one clarifying question (Section 0.C). Never dump a question list, and never guess a vibe.
- **No design system fits**: say so explicitly and justify the custom direction against Appendix B sources rather than reinventing tokens silently.
- **Constraint conflicts with aesthetics**: accessibility, public-sector, regulated, and trust-first constraints override taste (Section 0.A.6).
- **Redesign with no usable brand assets**: fall back to the audit protocol in Section 11 before proposing an overhaul.
- **Pre-flight check fails**: fix the flagged item; do not ship with a known Section 9 tell.

## Examples

Design reads that drive everything downstream:
...
kinetic-type language, leaning toward native CSS + scroll-driven animation.

Reading this as: redesign of a public-sector service site, with a trust-first
language, leaning toward GOV.UK Frontend or USWDS.
```

Dial configuration for a restrained B2B landing:
